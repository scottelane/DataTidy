...
namespace ScottLane.DataTidy.Dynamics365
{
    ...
    public class Dynamics365LookupValue : LookupValue, IDynamics365TargetEntitiesProvider
    {
        ...
        public Dynamics365LookupValue(IOperation parentOperation) : base(parentOperation)
        {
            RefreshBrowsableFields();
        }
        ...
        public override ValidationResult Validate()
        {
            ...
            result.AddErrorIf(((Dynamics365Field)DestinationField)?.Targets?.Length > 1 && targetMode == TargetMode.Fixed && TargetEntity == default(Dynamics365Entity), string.Format("Please specify a Target Entity for the '{0}' field value mapping", DestinationField.DisplayName));
            result.AddErrorIf(((Dynamics365Field)DestinationField)?.Targets?.Length > 1 && targetMode == TargetMode.Variable && TargetEntityField == default(Field), string.Format("Please specify a Target Entity Field for the '{0}' field value mapping", DestinationField.DisplayName));
            return result;
        }
