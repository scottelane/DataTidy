...
{
    public class Dynamics365UserProvidedValue : UserProvidedValue, IDynamics365TargetEntitiesProvider
    {
        private const TargetMode DEFAULT_TARGET_MODE = TargetMode.Fixed;

        [GlobalisedDisplayName(typeof(Dynamics365UserProvidedValue), nameof(DestinationField)), GlobalisedDecription(typeof(Dynamics365UserProvidedValue), nameof(DestinationField)), TypeConverter(typeof(Dynamics365FieldConverter)), JsonProperty(Order = 1)]
        ...
        [GlobalisedCategory("General"), GlobalisedDisplayName("Target Mode"), GlobalisedDecription("Indicates whether the Target Entity will be used for all records, or whether the entity varies based on the entity logical name specified in the Target Entity Field."), DefaultValue(DEFAULT_TARGET_MODE), Browsable(false), JsonProperty(Order = 2)]
        ...
        /// </summary>
        [GlobalisedCategory("General"), GlobalisedDisplayName("Target Entity"), GlobalisedDecription("The lookup target entity."), TypeConverter(typeof(Dynamics365TargetEntityConverter)), Browsable(false), JsonProperty(Order = 3)]
        ...
        /// <summary>
        /// Initialises a new instance of the Dynamics365UserProvidedValue class with the specified parent operation.
        /// </summary>
        /// <param name="parentOperation">The parent operation.</param>
        public Dynamics365UserProvidedValue(IOperation parentOperation) : base(parentOperation)
        { }
        ...
        public override ValidationResult Validate()
        {
            ...
            result.AddErrorIf(((Dynamics365Field)DestinationField)?.Targets?.Length > 1 && targetMode == TargetMode.Fixed && TargetEntity == default(Dynamics365Entity), string.Format("Please specify a Target Entity for the '{0}' field value mapping", DestinationField?.DisplayName ?? "<Unknown>"));
            result.AddErrorIf(((Dynamics365Field)DestinationField)?.Targets?.Length > 1 && targetMode == TargetMode.Variable && TargetEntityField == default(Field), string.Format("Please specify a Target Entity Field for the '{0}' field value mapping", DestinationField?.DisplayName ?? "<Unknown>"));
            return result;
        }
